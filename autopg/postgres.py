...
def write_sql_init_file(sql_content: str, filename: str) -> tuple[bool, Path | None]:
    """
    Write SQL initialization file, preferring Docker entrypoint directory if available.

    """
    if not sql_content.strip():
        return False, None

    docker_init_dir = Path("/docker-entrypoint-initdb.d")

    # Try to write to Docker entrypoint directory if it exists
    if docker_init_dir.exists() and docker_init_dir.is_dir():
        init_sql_path = docker_init_dir / filename
        try:
            with open(init_sql_path, "w") as f:
                f.write(f"-- Generated by AutoPG\n\n{sql_content}")
            return True, init_sql_path
        except (OSError, IOError):
            # Fall through to non-Docker behavior if write fails
            pass

    # In non-Docker environments or if Docker write failed, return SQL content
    return False, None
