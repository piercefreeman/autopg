...
@click.group()
@click.option('--host', default=lambda: os.getenv('POSTGRES_HOST', 'localhost'),
              help='PostgreSQL host')
@click.option('--port', default=lambda: int(os.getenv('POSTGRES_PORT', '5432')),
              help='PostgreSQL port')
@click.option('--database', default=lambda: os.getenv('POSTGRES_DB', 'benchmark'),
              help='PostgreSQL database name')
@click.option('--user', default=lambda: os.getenv('POSTGRES_USER', 'postgres'),
              help='PostgreSQL username')
@click.option('--password', default=lambda: os.getenv('POSTGRES_PASSWORD', 'postgres'),
              help='PostgreSQL password')
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose output')
@click.pass_context
def cli(ctx: click.Context, host: str, port: int, database: str, user: str, 
        password: str, verbose: bool) -> None:
...
@click.option('--table', default='posts',
              type=click.Choice(['users', 'posts', 'comments', 'events']),
              help='Table to scan')
@click.option('--limit', default=None, type=int, help='LIMIT clause for scans')
@click.option('--workers', '-w', default=1, help='Number of concurrent workers')
@click.pass_context
def seqscan(ctx: click.Context, iterations: int, table: str, limit: Optional[int], 
            workers: int) -> None:
