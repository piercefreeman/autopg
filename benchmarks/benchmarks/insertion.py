...
class AsyncInsertionBenchmark:
    ...
    async def run(self, table_name: str, num_records: int, batch_size: int = 1000, 
        ...
        # Run benchmark
        async with timed_operation(f"Async insertion benchmark ({num_workers} workers)", self.verbose) as timing:
            if num_workers == 1:
                batch_times = await self._run_single_connection(config['query'], batches)
            else:
                batch_times = await self._run_multi_connection(config['query'], batches, num_workers)
    ...
    async def _get_reference_data(self) -> Dict[str, List[int]]:
        ...
        try:
            async with AsyncDatabaseConnection(**self.db_config) as db:
                # Get user IDs
                user_result = await db.execute("SELECT id FROM benchmark.users ORDER BY id LIMIT 1000")
                reference_data['user_ids'] = [row['id'] for row in user_result]
                
                # Get post IDs
                post_result = await db.execute("SELECT id FROM benchmark.posts ORDER BY id LIMIT 1000")
    ...
    async def _run_multi_connection(self, query: str, batches: List[List[Tuple]], 
        ...
        async with AsyncConnectionPool(self.db_config, num_workers) as pool:
            with Progress(
                ...
                tasks = [
                    execute_batch_with_semaphore(batch, i)
                    for i, batch in enumerate(batches)
                ]
    ...
    async def _execute_batch(self, pool: AsyncConnectionPool, query: str, 
                           batch: List[Tuple], batch_idx: int) -> float:
...
class InsertionBenchmark:
    ...
    def run(self, table_name: str, num_records: int, batch_size: int = 1000, 
        ...
        return asyncio.run(self.async_benchmark.run(table_name, num_records, batch_size, num_workers))